...
/**
 * 产品站的 layout：页头 / 导航 / 主体 / 页脚，以及产品侧的 Provider 组合。
 *
 * `(product)` 是 route group，不参与 URL 构成——这里的每个子路由 URL
 * 与移动前逐字符一致。
 *
 * 只有这一层挂 `PrivyProviders`。harness 在 `(harness)` 下另有一套，
 * 两者永不嵌套（同 appId 的两个 PrivyProvider 实例会争抢同一批
 * `privy:` localStorage key，SDK 行为未定义）。
 */
