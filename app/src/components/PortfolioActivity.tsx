...
export function TradeRow({trade}: {trade: PortfolioTrade}) {
  ...
  return (
    <tr className="border-t border-border align-top">
      ...
      <td className="px-3 py-3"><p className="font-medium text-foreground">{chainLabel(trade.chain)} · {shortAddr(trade.token, 7, 6)}</p><p className="mt-1 font-mono text-[10px] text-muted">Trade {shortAddr(trade.trade_id, 8, 6)}{trade.cycle_opened_entry_id !== '0' ? ` · cycle ${trade.cycle_opened_entry_id}` : ' · unassigned'}</p></td>
      <td className="px-3 py-3 font-mono text-xs"><p>{trade.amount_in_actual ?? '—'} raw</p><p className="mt-1 text-[10px] text-muted">{shortAddr(paidToken, 7, 6)}</p></td>
      <td className="px-3 py-3 font-mono text-xs"><p>{trade.amount_out ?? '—'} raw</p><p className="mt-1 text-[10px] text-muted">{shortAddr(receivedToken, 7, 6)}</p></td>
      ...
      <td className="px-3 py-3 font-mono text-xs text-muted"><p>{trade.tx_chain ? chainLabel(trade.tx_chain) : '—'}</p><p className="mt-1" title={trade.tx_hash}>{shortAddr(trade.tx_hash, 8, 6)}</p></td>
    </tr>
  );
}
