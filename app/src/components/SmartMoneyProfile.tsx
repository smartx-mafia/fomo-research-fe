...
import {getSmartMoneyHoldings, getSmartMoneyTokenTrades, getSmartMoneyTrades, type SmartMoneyHolding, type SmartMoneyTokenTrades} from '@/api/smartmoney';
import {getPlatformUserPositionTrades, type PlatformTrade} from '@/api/platform-trades';
...
function HoldingSummary({entry, chain, variant}: {entry: SmartMoneyHolding; chain: string; variant: 'open'|'closed'}) {
  const token = entry.token_address;
  return <><div className="mb-3 flex flex-wrap gap-6 text-xs"><span>当前持仓价值 <b>{money(entry.usd_value)}</b></span><span>总盈亏 <b className={tone(entry.total_profit)}>{money(entry.total_profit)}</b></span><span>总市值 <SmartMoneyTokenFdv chain={entry.chain || chain} address={token} /></span>{variant === 'closed' ? <span title="平均持仓成本乘以采集时点总供应量，并非历史买入时市值">平均买入市值 <b>{money(entry.avg_cost_market_cap_usd)}</b></span> : null}<span>累积买入 <b>{money(entry.history_bought_cost)}</b></span><span>买入均价 <b>{price(entry.avg_bought_price)}</b></span>{variant === 'closed' ? <><span>累积卖出 <b>{money(entry.history_sold_income)}</b></span><span>累积卖出份额 <b>{qty(entry.history_sold_amount)}</b></span><span title="累积卖出金额 ÷ 累积卖出份额（USD/枚）">卖出均价 <b>{price(averageSellPrice(entry.history_sold_income, entry.history_sold_amount))}</b></span></> : null}</div>{entry.valuation_consistent === false ? <p className="mb-3 text-xs text-accent">估值数据存在差异，金额按来源展示。</p> : null}</>;
}
...
function TokenHistory({chain, address, token, entry, variant}: {chain: string; address: string; token: string; entry: SmartMoneyHolding; variant: 'open'|'closed'}) {
  ...
  return <div className="border-t border-border bg-background/40 p-4">
    <HoldingSummary entry={entry} chain={chain} variant={variant} />
...
/**
 * 用户级单仓位交易历史（new-position-trades，不分页）。
 *
 * 行按时间倒序、跨钱包混合；轮次由后端按钱包与 Token 独立编号，因此展示
 * 钱包列并按 `#round` 标注，不能把不同钱包的同号轮次当成同一轮。
 *
 * `positionID` 来自持仓行的 `position_id` 原样回传（字段缺失时才回退拼
 * `chain:token_address`）—— 格式属于后端，前端不复制这份知识。
 */
function PositionTrades({userID, positionID}: {userID: string; positionID: string}) {
  const history = useSWR(['platform-position-trades', userID, positionID], () => getPlatformUserPositionTrades(userID, positionID), {revalidateOnFocus: false, shouldRetryOnError: false});
  const rows = history.data?.list ?? [];
  return <div className="mt-2">
    {history.error ? <p className="py-4 text-sm text-down">交易历史加载失败。<button type="button" onClick={() => void history.mutate()} className="ml-2 underline">重试</button></p> : null}
    {history.isLoading ? <p className="py-4 text-sm text-muted">正在加载该仓位的用户交易历史…</p> : null}
    {!history.isLoading && !history.error && rows.length === 0 ? <p className="py-4 text-sm text-muted">该仓位暂无交易历史。</p> : null}
    {rows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-xs"><thead className="text-muted"><tr><th className="p-2 text-left">时间</th><th className="p-2 text-left">类型</th><th className="p-2 text-left">钱包</th><th className="p-2 text-right">数量</th><th className="p-2 text-right">计价</th><th className="p-2 text-right">成交价</th><th className="p-2 text-right">金额</th><th className="p-2 text-right">轮次</th><th className="p-2 text-right">Tx</th></tr></thead><tbody>{rows.map((row, index) => <tr key={`${row.wallet_address ?? ''}:${row.tx_hash ?? ''}:${index}`} className="border-t border-border"><td className="p-2">{when(row.occurred_at)}</td><td className="p-2"><EventBadge type={row.event_type} />{(row.legs ?? 0) > 1 ? <span className="ml-1 text-muted">×{row.legs}</span> : null}{row.round_close ? <span className="ml-1 rounded bg-surface-2 px-1 text-[10px] text-muted">清仓</span> : null}</td><td className="p-2 font-mono" title={row.wallet_address}>{shortAddr(row.wallet_address ?? '', 6, 4)}</td><td className="p-2 text-right font-mono">{qty(row.token_amount)}</td><td className="p-2 text-right font-mono">{qty(row.quote_amount)} {row.quote_symbol ?? ''}</td><td className="p-2 text-right font-mono">{price(row.price_usd)}</td><td className="p-2 text-right font-mono">{money(row.cost_usd)}</td><td className="p-2 text-right">{row.round ? `#${row.round}` : '—'}</td><td className="p-2 text-right font-mono" title={row.tx_hash}>{shortAddr(row.tx_hash ?? '', 6, 4) || '—'}</td></tr>)}</tbody></table></div> : null}
    {rows.length ? <p className="mt-3 text-xs text-muted">轮次按钱包与 Token 独立编号，同一轮次号可能来自不同钱包。</p> : null}
    {history.data?.coverage && history.data.coverage !== 'complete' ? <p className="mt-2 text-xs text-accent">该仓位的交易覆盖不完整，仅展示本地已采集记录。</p> : null}
  </div>;
}

export function SmartMoneyHoldingsTable({list, variant, chain, address, userID}: {list: (SmartMoneyHolding & {position_id?: string})[]; variant: 'open'|'closed'; chain: string; address?: string; userID?: string}) {
  ...
  return <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-xs"><thead className="text-muted"><tr><th className="p-3">Token</th><th className="min-w-[130px] px-6 py-3" aria-label={variant === 'closed' ? '总盈亏与ROI' : '当前持仓价值与ROI'} />{variant === 'open' ? <th className="p-3 text-right">持仓量</th> : null}<th className="p-3 text-right">已实现</th><th className="p-3 text-right">未实现</th><th className="p-3 text-right">总盈亏</th></tr></thead><tbody>{list.map((entry, index) => {
    ...
    const token = entry.token_address ?? ''; const rowKey = JSON.stringify([entry.chain || chain, token]); const open = token !== '' && expanded === rowKey;
    return <Fragment key={`${rowKey}:${index}`}><tr onClick={() => token && setExpanded(open ? undefined : rowKey)} className="cursor-pointer border-t border-border hover:bg-surface-2/50"><td className="p-3"><div className="flex items-center gap-2"><span className={`text-muted transition ${open ? 'rotate-90' : ''}`}>›</span><Token entry={entry} chain={chain} showAverage={variant === 'open'} tradeTime={variant === 'closed' ? when(entry.last_active_at) : undefined} /></div></td><td className="min-w-[130px] px-6 py-3 text-right font-mono font-semibold"><div className="flex flex-col items-end gap-1"><span title={variant === 'closed' ? '总盈亏（USD）' : '当前持仓价值（USD）'} className={`whitespace-nowrap ${variant === 'closed' ? tone(entry.total_profit) : 'text-foreground'}`}>{money(variant === 'closed' ? entry.total_profit : entry.usd_value)}</span><span title="ROI：总盈亏 ÷ 生涯累计买入成本" className={`text-[10px] ${roi.direction > 0 ? 'text-up' : roi.direction < 0 ? 'text-down' : 'text-muted'}`}>{roi.text}</span></div></td>{variant === 'open' ? <td className="p-3 text-right font-mono">{qty(entry.balance)}</td> : null}<td className={`p-3 text-right font-mono ${tone(entry.realized_profit)}`}>{money(entry.realized_profit)}<p className="text-[10px]">{ratio(entry.realized_profit_pnl)}</p></td><td className={`p-3 text-right font-mono ${tone(entry.unrealized_profit)}`}>{money(entry.unrealized_profit)}<p className="text-[10px]">{ratio(entry.unrealized_profit_pnl)}</p></td><td className={`p-3 text-right font-mono font-semibold ${tone(entry.total_profit)}`}>{money(entry.total_profit)}</td></tr>{open ? <tr><td colSpan={variant === 'open' ? 6 : 5} className="p-0"><>{address ? <TokenHistory chain={entry.chain || chain} address={address} token={token} entry={entry} variant={variant} /> : <div className="border-t border-border bg-background/40 p-4"><HoldingSummary entry={entry} chain={entry.chain || chain} variant={variant} />{userID ? <PositionTrades userID={userID} positionID={entry.position_id || `${entry.chain || chain}:${token}`} /> : <p className="text-xs text-muted">用户聚合交易明细暂未接入。</p>}</div>}</></td></tr> : null}</Fragment>;
  })}</tbody></table></div>;
}

/** 逐腿交易表；用户聚合模式（source）额外展示来源链与钱包，同一 tx 的多条腿不去重。 */
export function PlatformTradesTable({list, source = false}: {list: PlatformTrade[]; source?: boolean}) {
  return <div className="overflow-x-auto"><table className={`w-full ${source ? 'min-w-[1100px]' : 'min-w-[900px]'} text-left text-xs`}><thead className="text-muted"><tr><th className="p-3">时间</th><th className="p-3">类型</th><th className="p-3">Token</th>{source ? <><th className="p-3">链</th><th className="p-3">钱包</th></> : null}<th className="p-3 text-right">数量</th><th className="p-3 text-right">计价</th><th className="p-3 text-right">成交价</th><th className="p-3 text-right">金额</th><th className="p-3 text-right">Tx</th></tr></thead><tbody>{list.map((trade, index) => <tr key={`${trade.tx_hash ?? ''}:${index}`} className="border-t border-border"><td className="p-3">{when(trade.occurred_at)}</td><td className="p-3"><EventBadge type={trade.event_type} /></td><td className="p-3"><Token entry={{token_address: trade.token_address, symbol: trade.token_symbol, logo: trade.token_logo}} chain={source ? trade.chain : undefined} /></td>{source ? <><td className="p-3 text-muted">{trade.chain ? chainName(trade.chain) : '—'}</td><td className="p-3 font-mono" title={trade.wallet_address}>{shortAddr(trade.wallet_address ?? '', 6, 4)}</td></> : null}<td className="p-3 text-right font-mono">{qty(trade.token_amount)}</td><td className="p-3 text-right font-mono">{qty(trade.quote_amount)} {trade.quote_symbol ?? ''}</td><td className="p-3 text-right font-mono">{price(trade.price_usd)}</td><td className="p-3 text-right font-mono">{money(trade.cost_usd)}</td><td className="p-3 text-right font-mono" title={trade.tx_hash}>{shortAddr(trade.tx_hash ?? '', 6, 4) || '—'}</td></tr>)}</tbody></table></div>;
}

export function SmartMoneyProfile({chain, address, backHref = '/leaderboard'}: {chain: string; address: string; backHref?: string}) {
  ...
  return <div className="space-y-5">
    <a href={backHref} className="text-sm text-muted hover:text-foreground">← 返回榜单</a>
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-surface">
      ...
      {tab === 'holdings' && !holdings.isLoading ? <><div className="flex gap-1 border-b border-border p-3"><button onClick={() => setHoldingTab('open')} className={`rounded px-3 py-1.5 text-xs ${holdingTab === 'open' ? 'bg-up/10 text-up' : 'text-muted'}`}>持仓中 {holdings.data?.open?.length ?? 0}</button><button onClick={() => setHoldingTab('closed')} className={`rounded px-3 py-1.5 text-xs ${holdingTab === 'closed' ? 'bg-surface-2 text-foreground' : 'text-muted'}`}>已清仓 {holdings.data?.closed?.length ?? 0}</button></div>{list.length ? <SmartMoneyHoldingsTable list={list} variant={holdingTab} chain={chain} address={address} /> : <p className="p-10 text-center text-sm text-muted">暂无{holdingTab === 'open' ? '持仓中' : '已清仓'}记录。</p>}</> : null}
      {tab === 'trades' && !trades.isLoading ? trades.data?.list?.length ? <PlatformTradesTable list={trades.data.list} /> : <p className="p-10 text-center text-sm text-muted">暂无最新交易。</p> : null}
    </section>
  </div>;
}
