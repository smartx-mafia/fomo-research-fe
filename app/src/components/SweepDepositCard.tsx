...
export function SweepDepositCard({bearer, ownerKey, identityMatched, addresses, resumeID, onProtectedError}: {bearer: string; ownerKey: string; identityMatched: boolean; addresses?: DepositAddress[]; resumeID?: string; onProtectedError: (error: unknown) => boolean}) {
  ...
  return (
    <section className="rounded-lg border border-border bg-surface p-4">
      ...
      {review ? <div className="mt-4 rounded-md border border-accent/40 bg-accent/5 p-3"><div className="flex items-center gap-2.5"><TokenAvatar chain={review.asset.chain} address={review.asset.token_address} size={32} fallbackSymbol={review.symbol} /><p className="text-sm text-foreground">Create a sweep for the complete latest balance of <strong>{review.symbol ?? shortAddr(review.asset.token_address)}</strong> on {chainLabel(review.asset.chain)}?</p></div><p className="mt-1 text-xs text-muted">Selected identity: <span className="font-mono">{review.asset.chain} · {review.asset.token_address}</span>. The backend controls recipient, refund path and minimum output.</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => setReview(undefined)} className="rounded border border-border px-3 py-2 text-xs text-muted">Cancel</button><button type="button" onClick={() => void confirmCreate()} className="rounded bg-accent px-3 py-2 text-xs font-semibold text-white">Create sweep intent</button></div></div> : null}
      <div className="mt-4 space-y-2">
        {candidates.length === 0 ? <p className="text-sm text-muted">{addresses === undefined ? 'Cash balances are temporarily unavailable. Existing sweep recovery remains available below.' : addresses.some((route) => route.deposit_mode === 'sweep' && route.balance_raw === undefined) ? 'Some EVM balances are unavailable. Refresh balances before checking sweep eligibility.' : 'No enabled EVM balance currently meets the sweep minimum. Refresh balances after a transfer.'}</p> : candidates.map((position) => {
          ...
          return (
            <div key={`${position.asset.chain}:${position.asset.token_address}`} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border bg-background p-3">
              <div className="flex items-center gap-2.5"><TokenAvatar chain={position.asset.chain} address={position.asset.token_address} size={32} fallbackSymbol={position.symbol} /><div><p className="text-sm font-medium text-foreground">{position.symbol || shortAddr(position.asset.token_address)} · {chainLabel(position.asset.chain)}</p><p className="mt-1 font-mono text-xs text-muted">{shortAddr(position.asset.token_address, 7, 6)}</p>{position.minAmountRaw ? <p className="mt-1 text-xs text-muted">Minimum: {formatBaseUnitsExact(position.minAmountRaw, position.decimals)} {position.symbol}</p> : null}</div></div>
